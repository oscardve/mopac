 ...
 *
 * (Edited for use in the R package "MoPAC" by Oscar Villarreal on 5/31/18.)
 */
...
double betain(double x, double p, double q, double beta, int *ifault){
  int indx{},ns{};
  double acu = 0.1E-14,ai{},cx{},pp{},psq{},qq{},rx{},temp{},term{},value{},xx{};
  value = x;
  *ifault = 0;
  /*
...
double BetaNoncentralCdf(double a, double b, double lambda, double x, double error_max){
  int i{},ifault{};
  double beta_log{},bi{},bj{},p_sum{},pb_sum{},pi{},pj{},si{},sj{},value{};
  i = 0;
  pi = exp(-lambda / 2.0);
  beta_log = LogGamma(a, &ifault) + LogGamma(b, &ifault) - LogGamma(a+b, &ifault);
  bi = betain(x, a, b, beta_log, &ifault);
  si = exp(a*log(x) + b*log(1.0-x) - beta_log - log(a));
  p_sum = pi;
  pb_sum = pi * bi;
